...
export default function AgencyCreditsPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/account/context')
      .then((r) => r.json())
      .then((data) => setRole(data?.member?.role || 'member'))
      .catch(() => {});
  }, [loadCredits]);
      ...
      fetch('/api/credits/verify', {
        ...
        .then((data) => {
          if (data.success) {
            ...
            window.history.replaceState({}, '', '/agency/credits');
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Top Up — only for owners and managers */}
      {role !== 'member' && (
        <div>
      ...
      {/* Members see a message instead of purchase section */}
      {role === 'member' && (
        <div className="p-6 rounded-xl border border-border bg-surface">
          <p className="text-sm text-muted">
            Credit purchases are managed by your workspace owner. Contact your admin if you need more credits allocated.
          </p>
        </div>
      )}
